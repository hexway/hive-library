...
from hive.rest import HiveRestApi
from hive.enum import RecordTypes
...
# Create project
new_project: Optional[Hive.Project] = hive_api.create_project(
    Hive.Project(name="test_project", description="test project")
)
print(f"New project: \n{new_project}\n")

# Get list of projects
projects_list: Optional[List[Hive.Project]] = hive_api.get_projects_list()
print(f"Projects list: \n{projects_list}\n")
project: Hive.Project = projects_list[0]

# Create host
new_host: Hive.Host = Hive.Host()
new_host.ip = IPv4Address("192.168.0.1")
new_host.ports = [
    Hive.Host.Port(
        port=80,
        service=Hive.Host.Port.Service(
...
new_host.names = [
    Hive.Host.Name(
        ...
        tags=[Hive.Tag(name="hostname_tag")],
    )
]
new_host.records = [
    Hive.Record(
...
new_host.tags = [Hive.Tag(name="host_tag")]
...
# Get list of hosts
hosts_list: Optional[List[Hive.Host]] = hive_api.get_hosts(project_id=project.id)
print(f"Hosts list: \n{hosts_list}\n")
host: Hive.Host = hosts_list[0]

# Create note
new_note: Optional[Hive.Note] = hive_api.create_note(
    note_text="test note text", project_id=project.id, node_id=host.id
)
print(f"New note: \n{new_note}\n")

# Get list of notes
updated_hosts_list: Optional[List[Hive.Host]] = hive_api.get_hosts(
    project_id=project.id
)
print(f"Notes list: \n{updated_hosts_list[0].notes}\n")

# Create file
new_file: Optional[Hive.File] = hive_api.upload_file(
...
# Get imported hosts
imported_hosts: Optional[List[Hive.Host]] = hive_api.get_hosts(project_id=project.id)
print(f"Imported hosts: \n{imported_hosts}\n")

# Delete project
deleted_project: Optional[Hive.Project] = hive_api.delete_project_by_name(
